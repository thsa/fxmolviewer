...
import org.openmolecules.fx.viewer3d.interactions.Interaction;
import org.openmolecules.fx.viewer3d.interactions.InteractionCalculator;
import org.openmolecules.fx.viewer3d.interactions.InteractionPoint;
import org.openmolecules.fx.viewer3d.interactions.InteractionSites;

import java.util.ArrayList;
import java.util.List;
import java.util.TreeMap;

public class PLIPInteractionCalculator implements InteractionCalculator {
...
//	private static final double PICATION_DIST_MAX = 6.0;     // original angle-independent value
	...
	public List<InteractionPoint> determineInteractionPoints(V3DMolecule fxmol) {
		ArrayList<InteractionPoint> interactionPointList = new ArrayList<>();
		...
		for (int ring=0; ring<mol.getRingSet().getSize(); ring++) {
			if (mol.getRingSet().isAromatic(ring)) {
				interactionPointList.add(new InteractionPoint(fxmol, mol.getRingSet().getRingAtoms(ring), IP_TYPE_AROMATIC_RING));
			}
		}

		for (int atom=0; atom<mol.getAtoms(); atom++) {
			if (mol.getAtomicNo(atom) == 6 && mol.getAtomZValue(atom) == 0) {
				interactionPointList.add(new InteractionPoint(fxmol, atom, IP_TYPE_HYDROPHOBIC));
			}

			if (mol.getAtomicNo(atom) == 7 || mol.getAtomicNo(atom) == 8) { // TODO S,P,Se?
				if (mol.getAllHydrogens(atom) != 0)
					interactionPointList.add(new InteractionPoint(fxmol, atom, IP_TYPE_DONOR));
				else if (mol.getAtomCharge(atom) <= 0)
					interactionPointList.add(new InteractionPoint(fxmol, atom, IP_TYPE_ACCEPTOR));
			}
			...
			if ((mol.getAtomicNo(atom) == 7
			 ...
			 && mol.getConnAtoms(atom) == 1) {
				...
				if (connAtomicNo == 6
				 ...
				 || connAtomicNo == 15
				 || connAtomicNo == 16)
					interactionPointList.add(new InteractionPoint(fxmol, atom, IP_TYPE_HALOGEN_ACCEPTOR));
			}

			if (mol.isHalogene(atom)
			 && mol.getConnAtoms(atom) == 1
			 && mol.getAtomicNo(mol.getConnAtom(atom, 0)) == 6) {
				interactionPointList.add(new InteractionPoint(fxmol, atom, IP_TYPE_HALOGEN_DONOR));
			}
			...
			if (isGuanidinCarbon(mol, atom)
			 ...
			 || isSulfoniumSulfur(mol, atom)) {
				interactionPointList.add(new InteractionPoint(fxmol, atom, IP_TYPE_POS_CHARGE));
			}

			if (isCarboxylCarbon(mol, atom)
			 || isPhosphatePhosphor(mol, atom)
			 || isSulfonateSulfur(mol, atom))
				interactionPointList.add(new InteractionPoint(fxmol, atom, IP_TYPE_NEG_CHARGE));
		}
	...
	public void determineInteractions(InteractionSites is1, InteractionSites is2, TreeMap<Integer,ArrayList<Interaction>> interactionMap) {
		...
		for (InteractionPoint p1: is1.getSites() ) {
			for (InteractionPoint p2: is2.getSites() ) {
				Interaction interaction = determineInteraction(p1,p2);
				if (interaction != null)
	...
	private Interaction determineInteraction(InteractionPoint ip1, InteractionPoint ip2) {
		...
		if (ip1.getType() == IP_TYPE_HYDROPHOBIC
		 && ip2.getType() == IP_TYPE_HYDROPHOBIC) {
			return distance<HYDROPH_DIST_MAX ? new Interaction(ip1, ip2, I_TYPE_HYDROPHOBIC, distance, 0.0, 1.0, INTERACTION_COLOR[I_TYPE_HYDROPHOBIC]) : null;
		}
		...
		if ((ip1.getType() == IP_TYPE_DONOR && ip2.getType() == IP_TYPE_ACCEPTOR)
		 || (ip2.getType() == IP_TYPE_DONOR && ip1.getType() == IP_TYPE_ACCEPTOR)) {
			if (distance<HBOND_DIST_MAX) {
				...
				InteractionPoint donIP = (ip1.getType() == IP_TYPE_DONOR) ? ip1 : ip2;
				...
				if (angle > HBOND_DON_ANGLE_MIN) {
					Interaction ia = new Interaction(ip1, ip2, I_TYPE_HBOND, distance, angle, 1.0, INTERACTION_COLOR[I_TYPE_HBOND]);
			...
			if (distance<HALOGEN_DIST_MAX) {
				...
				if (Math.abs(angle1 - HALOGEN_DON_ANGLE) < HALOGEN_ANGLE_DEV) {
					...
					if (Math.abs(angle2 - HALOGEN_ACC_ANGLE) < HALOGEN_ANGLE_DEV)
						return new Interaction(ip1, ip2, I_TYPE_HALOGEN_BOND, distance, 0.0, 1.0, INTERACTION_COLOR[I_TYPE_HALOGEN_BOND]);
				}
			}
		}
		...
		if ((ip1.getType() == IP_TYPE_POS_CHARGE && ip2.getType() == IP_TYPE_NEG_CHARGE)
		 || (ip2.getType() == IP_TYPE_POS_CHARGE && ip1.getType() == IP_TYPE_NEG_CHARGE)) {
			if (distance<SALTBRIDGE_DIST_MAX)
				return new Interaction(ip1, ip2, I_TYPE_SALT_BRIDGE, distance, 0.0, 1.0, INTERACTION_COLOR[I_TYPE_SALT_BRIDGE]);
		}
				...
				if (((angle < PISTACK_ANG_DEV)
				 ...
				 &&	((projectedDistanceOnPlane(p1, p2, n2l) < PISTACK_OFFSET_MAX)
				  || (projectedDistanceOnPlane(p2, p1, n1l) < PISTACK_OFFSET_MAX)))
					return new Interaction(ip1, ip2, I_TYPE_PI_STACKING, distance, angle, 1.0, INTERACTION_COLOR[I_TYPE_PI_STACKING]);
			}
			return null;
		}
	...
	private void removeRedundantHydrophobicInteractions(TreeMap<Integer,ArrayList<Interaction>> interactionMap) {
		...
		ArrayList<Interaction> redundant = new ArrayList<>();
		for (Interaction hydrophobic : interactionMap.get(I_TYPE_HYDROPHOBIC)) {
		...
		for (int site=0; site<2; site++) {
			TreeMap<Integer,ArrayList<Interaction>> atomInteractionMap = new TreeMap<>();
			for (Interaction hydrophobic : interactionMap.get(I_TYPE_HYDROPHOBIC)) {
				int atom = hydrophobic.getInteractionPoint(site).getAtom();
				ArrayList<Interaction> atomInteractions = atomInteractionMap.get(atom);
	...
	private void removeRedundantHBondInteractions(TreeMap<Integer,ArrayList<Interaction>> interactionMap) {
		ArrayList<Interaction> redundant = new ArrayList<>();
		for (Interaction hbond : interactionMap.get(I_TYPE_HBOND)) {
			int atom1 = hbond.getInteractionPoint(0).getAtom();
			int atom2 = hbond.getInteractionPoint(1).getAtom();

			boolean found = false;

			for (Interaction saltBridge : interactionMap.get(I_TYPE_SALT_BRIDGE)) {
		...
		ArrayList<Interaction> allHBonds = interactionMap.get(I_TYPE_HBOND);
		ArrayList<Interaction> bestHBonds = new ArrayList<>();
		for (int site=0; site<2; site++) {
			TreeMap<Integer,ArrayList<Interaction>> atomInteractionMap = new TreeMap<>();
			for (Interaction hbond : allHBonds) {
				InteractionPoint ip = hbond.getInteractionPoint(site);
				if (ip.getType() == IP_TYPE_DONOR) {
					ArrayList<Interaction> atomInteractions = atomInteractionMap.get(ip.getAtom());
					if (atomInteractions == null) {
						atomInteractions = new ArrayList<>();
			...
			for (int atom : atomInteractionMap.keySet()) {
				ArrayList<Interaction> hbondsWithAtomAsDonor = atomInteractionMap.get(atom);
				Interaction bestHBond = hbondsWithAtomAsDonor.get(0);
				double bestAngle = bestHBond.getAngle();
				for (int i=1; i<hbondsWithAtomAsDonor.size(); i++) {
					Interaction hbond = hbondsWithAtomAsDonor.get(i);
