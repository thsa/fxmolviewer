...
//          checkNanInf(state.getResult()); // in case of NaN or INF return BLACK instead; TLS 1-Mar-2016
            if (checkNanInf(state.getResult()))
                state.setResult(Color.BLACK);
            return state;
        } else
            return null;
    ...
    private static final boolean checkNanInf(Color c) {
        if (c.isNan()) {
