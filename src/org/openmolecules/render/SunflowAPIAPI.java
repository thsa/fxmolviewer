...
public class SunflowAPIAPI {
	...
	private BufferedImageDisplay bufferedImageDisplay;
	...
	/**
	 * Using the current colorSpace this converts the AWT-color color to a sunflow color
	 * that includes the power value, i.e. the light intensity. TLS 19Nov2013
	 * @param color
	 * @param power
	 * @return
	 */
	private float[] toSunFlowColor(java.awt.Color color, float power) {
		float f = power / 255f;
		try {
			return ColorFactory.createColor(colorSpace, f*color.getRed(), f*color.getGreen(), f*color.getBlue()).mul(power).getRGB();
			}
	 ...
	 */
	public void setDirectionalLight(String name, Point3 source, Vector3 direction, float radius, Color color, float intensity) {
		sunflow.parameter("source", source);
		sunflow.parameter("dir", direction);
		sunflow.parameter("radius", radius);
		sunflow.parameter("radiance", null, toSunFlowColor(color, intensity));
	 ...
	 * @param intensity light intensity
	 */
	public void setPointLight(String name, Point3 center, Color color, float intensity) {
		sunflow.parameter("center", center);
		sunflow.parameter("power", null, toSunFlowColor(color, intensity));
	 ...
	 */
	public void setSphereLight(String name, Point3 center, Color color, float intensity, int samples, float radius) {
		sunflow.parameter("center", center);
		sunflow.parameter("radiance", null, toSunFlowColor(color, intensity));
		sunflow.parameter("samples", samples);
		sunflow.parameter("radius", radius);
	 ...
	 */
	public void drawMeshLight(String name, Color color, float intensity, int samples, float[] vertices, int[] triangles) {
...
//		set shader
	 ...
	 * @param eta ?
	 */
	public void setGlassShader(String name, Color color, float eta) {
//		save name for use with primitives
		currShader = name;
...
//		set shader
	 ...
	 * @param zRotation z rotation
	 */
	public void drawMesh(String name, float[] vertices, int[] triangles, float size, float x, float y, float z, float xRotation, float yRotation, float zRotation) {
...
//    	 vertex amount
        ...
        for(int i=0;i<trianglesLength;i++) {
            ...
            if(runx == 1) triangles[i] = i-2;//mImage; // point 2
            if(runx == 2) triangles[i] = i;// mImage+1; // point 3
...
//		rendering options
	...
	public BufferedImage renderBufferedImage() {	// added this method; TLS 20130609
		sunflow.parameter("camera", currCamera);
		sunflow.parameter("resolutionX", width);
		sunflow.parameter("resolutionY", height);
		sunflow.parameter("aa.min", aaMin);
		...
		sunflow.parameter("filter", currFilter);
		sunflow.options(SunflowAPI.DEFAULT_OPTIONS);
		bufferedImageDisplay = new BufferedImageDisplay();
		sunflow.render(SunflowAPI.DEFAULT_OPTIONS, bufferedImageDisplay);
		return bufferedImageDisplay.getImage();
	}
...
//		rendering options
	...
	public void setBasicScene(int sceneWidth, int sceneHeight) {
		...
		this.setPointLight("myPointLight", new Point3(0,5,5), new Color(255,255,255), 100f);
		this.setDirectionalLight("myDirectionalLight", new Point3(-2,3,0), new Vector3(0,0,0), 3, new Color(1f,0f,0f), 100f);
		// set shader
